...
package fl.video {
	...
	CONFIG const FLASHPROONLY = true;
	 ...
	 * when a new media file is loaded or when one of those two properties is changed.
	 * 
	...
	/**
	 * Dispatched when playing completes because the player reached the end of the media file. 
	 ...
	 * <code>totalTime</code>
	 * property explicitly, and it downloads a media file that does not specify the duration 
	 ...
	 * <code>NetStream.onCuePoint</code> event callback for media file cue points. 
	...
	/**
	 * Dispatched the first time the media file's metadata is reached.
	 ...
	 * in some cases when the media file is loaded and the <code>autoPlay</code> property 
	...
	/**
	 * Dispatched while the media file is playing at the frequency specified by the 
	...
	/**
	 * Dispatched when a media file is loaded and ready to display. It starts the first 
	 * time you enter a responsive state after you load a new file with the <code>play()</code>
	 * or <code>load()</code> method. It starts only once for each file that is loaded.
	 * 
	...
	/**
	 * Dispatched when the user stops scrubbing a file with the seek bar. 
	 * Scrubbing refers to grabbing the handle of the seek bar and dragging it in 
	 * either direction to locate a particular scene in a file. Scrubbing stops 
	...
	/**
	 * Dispatched when the user begins scrubbing a file with the seek bar. 
	 * Scrubbing refers to grabbing the handle of the seek bar and dragging it in either direction 
	 * to locate a particular scene in a file. 
	 * Scrubbing begins when the user clicks the seek bar handle and ends when the user 
	 * releases it.
	 * 
	 ...
	 * specified time even if the source file doesn't have a keyframe there.) Second, 
	 ...
	 * does not begin playing a file until the <code>ready</code> event 
	 * and <code>skinLoaded</code> (or <code>skinError</code>) events have both started.
	 * 
	 ...
	 * instead) when the file is loaded. The FLVPlayback instance also dispatches this 
	 * event when the playhead stops at the end of the file
	 ...
	 * 
     * <p>The FLVPlayback class allows you to include a video player in your application. The video player can
     * play progressively downloaded video files over HTTP or it can stream video files 
     * from Flash Media Server or Flash Video Streaming Service (FVSS) providers.</p>
     *
     * <p>The FLVPlayback 2.5 component adds support for DVR playback and dynamic streaming when you use the component as a client  
     * for a Flash Media Server 3.5 application. The DVR and dynamic streaming features require Flash Player 10 or AIR 1.5. 
     * To enable DVR playback in a live stream, set the <code>FLVPlayback.isDVR</code> property
     * to <code>true</code>. To set up dynamic streaming, see the DynamicStream class. Dynamic streaming delivers to the FLVPlayback 2.5 component
     * the highest bitrate stream that can play smoothly based on client conditions.</p>
     *  
     ...
     * 
     * <p>To access the properties, methods, and events of the FLVPlayback class, import the
     * class to your application. You can either drag the FLVPlayback component to the Stage or
     * use the ActionScript <code>import</code> statement.
	...
	public class FLVPlayback extends Sprite {
		...
		public function FLVPlayback() {
			...
			// tyounsi we want to play video on stage
 CONFIG::FLASHPROONLY {
                         isLivePreview = false;
}
		...
		/**
		 * Begins loading the media file and provides a shortcut for setting the 
		 ...
		 * does nothing.
		 *
		 * @param source A string that specifies the URL of the file to stream 
		 * and how to stream it. The URL can be a local path, an HTTP URL to an media file, 
		 * an RTMP URL to a media file, an HTTP URL to an XML file, or, if you're using dynamic streaming, 
         * the path to a SMIL file.
		 * 
		...
		/**
		 * Plays the video stream. If you don't pass any arguments, the method switches the 
		 * file from a paused or stopped state to a playing state.
		 * 
		 * <p>If you pass arguments, the method acts as a shortcut for setting the 
		 ...
		 * they are not set. </p>
		 *
		 * <p>When waiting for enough of a progressive download file to load before playing
		 * starts automatically, call the <code>play()</code> method with no parameters
		 * to force playback to start immediately.</p>
		 * 
		 * @param source A string that specifies the URL of the file to stream 
		 * and how to stream it. The URL can be a local path, an HTTP URL to a file, 
		 * an RTMP URL to a file on Flash Media Server, an HTTP URL to an XML file, or, if you're using
         * dynamic streaming, a SMIL file. This parameter must be set, 
		 * either here or in the Component Inspector.
		 * 
		 * @param totalTime A number that is the total playing time for the video. Optional.
		 * 
		 * @param isLive A Boolean value that is <code>true</code> if the video stream is live. 
		...
		/**
		 * Plays a stream using the dynamic streaming feature.  To use dynamic streaming, create a 
		 * DynamicStreamItem and pass it as an argument to the <code>play2()</code> method.  The DynamicStreamItem 
         * contains streams encoded at different bitrates. The component
		 * automatically switches between the streams to 
		 * deliver the highest quality stream that can play smoothly.
		 * 
		 * @param dsi A DynamicStreamItem object containing the connection URI, streams, and stream bit rates.
         *
         * @langversion 3.0
		...
		/**
		 * Plays the file when enough of it has downloaded. If the file has 
		 * downloaded completely or it is streaming from Flash Media Server (FMS) calling the 
		 * <code>playWhenEnoughDownloaded()</code> method
		 * is identical to calling the <code>play()</code> method with no parameters.  Calling this
	 ...
	 * @tiptext pause method
		 ...
		 * time even if the source file doesn't have a keyframe there.) Second, 
		 * seeking is asynchronous, so if you call a seek method or set the <code>playheadTime</code> 
		 ...
		 * to the precise specified time even if the source file 
		 ...
		 * 
		 * @param A number that specifies a percentage of the length of the file 
		 ...
		 * operation to the point representing that percentage of the file's playing time.
		 * 
		 ...
		 * been disabled and goes to the end of the file if there is no other cue point.
		 *
		 * <p>For several reasons, the <code>playheadTime</code> property might not have the 
		 * expected value immediately after you call one of the seek methods or 
		 * set <code>playheadTime</code> to cause seeking. First, for a progressive 
		 ...
		 * set. To set cue point information for the next file to be loaded, 
		...
		/**
		 * Removes an ActionScript cue point from the currently loaded file. 
		 ...
		 *
		 * <p>Cue point information is wiped out when the <code>source</code> property is 
		 * set, so to set cue point information for the next file to be loaded, set the 
		...
		/**
		 * Enables or disables one or more cue points. Disabled cue points 
		 ...
		 * property to a different file, so set the <code>source</code> property before 
		 ...
		 * or disable (<code>false</code>) an file cue point.
		 * 
		...
		/**
		 * Returns <code>false</code> if the file embedded cue point is disabled. 
    ...
    *
    * <p><strong>Player Version</strong>: Flash Player 9.0.115.0.</p>
    *
	...
	*/
		 ...
		 * Use this property to manage multiple streams. 
		 * 
		 ...
		 * 
		 * <p>To load a second file in the background, set <code>activeVideoPlayerIndex</code>
		 ...
		 * the file loads but does not start playing until the <code>play()</code>
		 ...
		 * when loading a file by progressive download, playback starts only
		 * when enough of the file has download so that the file can play from start
		 * to finish. </p>
		 * 
		 * <p>To force playback before enough of the file has downloaded,
		 ...
		 * <code>playWhenEnoughDownloaded()</code> method.</p>
		 * 
		 * <p>If you set the property to <code>true</code> between the loading of new files, 
		...
		/**
		 * A Boolean value that, if <code>true</code>, causes the file to rewind to Frame 1 when 
	...
	/**
	 * The bits per second at which to transfer the file.
	 *
	 ...
	 */
		 ...
		 * file is in the buffering state. This can be useful to prevent a 
		 * user from using these controls to try to speed up playing the file 
		 ...
		 * beginning to play a video stream. For files streaming over RTMP, 
		 ...
		 * downloaded file over HTTP, there is little benefit to increasing this 
		 ...
		 * 
		 * <p>This property does not specify the amount of the file to download before 
		 * starting playback. </p>
		 * 
		...
		/**
		 * A string that specifies the URL of the file to stream and how to stream it.
		 * The URL can be an HTTP URL to a file, an RTMP URL to a stream on Flash Media Server, an 
		 * HTTP URL to an XML file, or a SMIL file (for use with dynamic streaming).
		 *
		 * <p>If you set this property through the Component inspector or the Property inspector, the 
		 * file begins loading and playing at the next "<code>enterFrame</code>" event.
		 ...
		 * <code>totalTime</code>, and <code>isLive</code> properties affect how the new file is 
		 ...
		 * file from playing automatically.</p>
		 *
		 * @see #autoPlay
		 * @see #isLive
		 * @see #totalTime
		...
		/**
		 * The number of seconds the component appends
		 * to a live stream when the stream is in DVR mode.  If the total
		 * duration of the DVR stream is not made available from the server, the 
		 * FLVPlayback component continually appends this value in minutes when the current 
		 * recorded duration is within the <code>dvrIncrementVariance</code> of the perpetual total
		...
		public function get dvrIncrement():Number {
			var vp:VideoPlayer = videoPlayers[_activeVP];
			if (isNaN(vp.dvrIncrement)) return NaN
			return vp.dvrIncrement;
		}
		...
		public function set dvrIncrement(increment:Number):void {
			var vp:VideoPlayer = videoPlayers[_activeVP];
			vp.dvrIncrement = increment;
		}
		
		[Inspectable(defaultValue=300)]
		/**
		 * The difference threshold, in seconds, between the current duration
		 * and total duration of a DVR stream.  When this value is reached, it 
		 * triggers a call to Flash Media Server to return the current and total
		 * duration of a stream.  If the total duration of the stream is unknown,
		 * the component appends the value of <code>dvrIncrement</code> to the current
		 * duration and calls the server when the difference threshold is reached again.
		 * 
		 * @see #isDVR
		 * @see #dvrIncrement
         *
		...
		public function get dvrIncrementVariance():Number {
			var vp:VideoPlayer = videoPlayers[_activeVP];
			if (isNaN(vp.dvrIncrementVariance)) return NaN
			return vp.dvrIncrementVariance;
		}
		...
		public function set dvrIncrementVariance(variance:Number):void {
			var vp:VideoPlayer = videoPlayers[_activeVP];
			vp.dvrIncrementVariance = variance;
		}
		
		[Inspectable(defaultValue=false)]
		/**
		 * If a DVR stream runs beyond the total duration defined
		 * by Flash Media Server, enabling <code>dvrFixedDuration</code> forces the stream to
		 * stop playing when the total duration is reached. If disabled, the server appends the value
		 * of <code>dvrIncrement</code> to the end of the stream.  This property
		 * is ignored when the total duration is not provided by Flash Media Server.
		 * 
		 * @see #isDVR
		 * @see #dvrIncrement
		 * @see #dvrIncrementVariance 
		...
		public function set dvrFixedDuration(fixed:Boolean):void {
			var vp:VideoPlayer = videoPlayers[_activeVP];
			vp.dvrFixedDuration = fixed;
		}
		
		[Inspectable(defaultValue=true)]
		/**
		 * By default, DVR streams begin playing at the beginning of the recording.
		 * Set this property to <code>true</code> moves the playhead
		...
		public function set dvrSnapToLive(snapToLive:Boolean):void {
			var vp:VideoPlayer = videoPlayers[_activeVP];
			vp.dvrSnapToLive = snapToLive;
		}
		
		/**
		 * Enables compatibility with the Flash Media Server LiveCast application.
		 *
         * @see VideoPlayer#enableLiveCast
         *
         * @langversion 3.0
         * @playerversion Flash 10
         * @playerversion AIR 1.5
         ...
         */
		public function set enableLiveCast(lc:Boolean):void {
			if(this.isDVR == true && lc == true) {
				return;
			}
			if(this.isLive == false && lc == true) {
				this.isLive = true;
			}
			var vp:VideoPlayer = videoPlayers[_activeVP];
			vp.enableLiveCast = lc;
		}
		...
		/**
		 * The amount of time, in milliseconds, before Flash Playre terminates an idle connection 
		 * to Flash Media Server because playing paused or stopped. This property has no effect on an 
		...
		[Inspectable(defaultValue=false)]
		/**
		 * Enables DVR functionality when a live stream is
		...
		/**
		 * A Boolean value that is <code>true</code> if the file is streaming from 
		 * Flash Media Server over RTMP. Its value is <code>false</code> for any other file source. 
		 *
		 ...
		 * is effective only when streaming from Flash Media Server or Flash Video Streaming Service (FVSS). The value of this 
		 * property is ignored for an HTTP download or if the <code>isDVR</code> property is <code>true</code>.
		 * 
		 * <p>If you set this property between loading new files, it has no 
		 * effect until the <code>source</code> property is set for the new file.</p>
		 * 
		 ...
		 * 
		 * <p>If the file is encoded with the Flash 8 encoder, the <code>metadata</code> 
		 * property contains the following information. Older files contain 
		 * only the <code>height</code>, <code>width</code>, and <code>duration</code> values.</p>
		 * 
		 * <table class="innertable" width="100%">
		 * 	<tr><th><b>Parameter</b></th><th><b>Description</b></th></tr>
		 * 		<tr><td><code>canSeekToEnd</code></td><td>A Boolean value that is <code>true</code> if the file is encoded with a keyframe on the last frame that allows seeking to the end of a progressive download movie clip. It is <code>false</code> if the file is not encoded with a keyframe on the last frame.</td></tr>
		 * 		<tr><td><code>cuePoints</code></td><td>An array of objects, one for each cue point embedded in the file. Value is undefined if the media file does not contain any cue points. Each object has the following properties:
	     *   	
		 * 			<ul>
		 ...
		 * <tr><td><code>audiodelay</code></td><td>A number that represents time <code>0</code> in the source file from which the media file was encoded. 
		 * <p>Video content is delayed for the short period of time that is required to synchronize the audio. For example, if the <code>audiodelay</code> value is <code>.038</code>, the video that started at time <code>0</code> in the source file starts at time <code>.038</code> in the media file.</p> 
		 * <p>Note that the FLVPlayback and VideoPlayer classes compensate for this delay in their time settings. This means that you can continue to use the time settings that you used in your the source file.</p>
 		 ...
 		 * <tr><td><code>framerate</code></td><td>A number that is the frame rate of the media file.</td></tr>
 		 * <tr><td><code>videodatarate</code></td><td>A number that is the video data rate of the media file.</td></tr>
		 * <tr><td><code>height</code></td><td>A number that is the height of the media file.</td></tr>
 		 * <tr><td><code>width</code></td><td>A number that is the width of the media file.</td></tr>
 		 * <tr><td><code>duration</code></td><td>A number that specifies the duration of the media file in seconds.</td></tr>
		 * </table>
		 *
		 ...
		 * encountered and processed or if the media file was encoded without the 
		...
		/**
         * A Boolean value that is <code>true</code> if the media file is in a paused state.
         *
         * @langversion 3.0
         * @playerversion Flash 9.0.28.0
		 */
		 ...
		 * while the media file plays, the component dispatches the <code>playheadUpdate</code>
		 * event.</p>
		 * 
		 * <p>For several reasons, the <code>playheadTime</code> property might not have the expected 
		 * value immediately after you call one of the seek methods or set <code>playheadTime</code> 
		 ...
		 * keyframe, so a seek takes you to the time of the first keyframe after the 
		 * specified time. (When streaming, a seek always goes to the precise specified 
		 * time even if the source media file doesn't have a keyframe there.) Second, 
		 ...
		 * <code>playheadUpdate</code> event. Setting this property while the media file is 
		...
		/**
		 * A Boolean value that is <code>true</code> if the media file is in the playing state. 
         *
         * @langversion 3.0
         * @playerversion Flash 9.0.28.0
		 */
		...
		/**
		 * A number that specifies the height of the source media file. This information 
		...
		/**
		 * Gives the width of the source media file. This information is not valid immediately 
		 ...
		 * to the dimensions of the source media file.  If set to
		 * <code>VideoScaleMode.EXACT_FIT</code>, causes the dimensions of
		 * the source media file to be ignored and the video is stretched to
		 * fit the rectangle defined by
		 * <code>registrationX</code>, <code>registrationY</code>,
		 * <code>registrationWidth</code> and
		 * <code>registrationHeight</code>. If this is set
		 * after a media file has been loaded an automatic layout will start
         ...
         * it in either direction to locate a particular scene in the media file.</p> 
         *
         * @langversion 3.0
         * @playerversion Flash 9.0.28.0
		 */
	...
	* appearance of the media file.</p>
	* 
	...
	* mode.</p>
	*
	* <p>If the media file is large (for example, 640 pixels wide or more, 480 pixels 
	...
	*/
		 ...
		 * automatically if the media file has metadata embedded; otherwise,
		 ...
		 * media file that is loaded by setting the <code>source</code> property. It has no effect
		 * on a media file that has already loaded.  Also, this property does not return 
		 * the new value passed in until a media file is loaded.</p>
		 *
		 ...
		 *
		 * @see #source
		 * @tiptext The total length of the media file in seconds
         * @helpid 3467
         *
         * @langversion 3.0
         * @playerversion Flash 9.0.28.0
		...
		/**
		 * A number that you can use to manage multiple streams. 
		...
		private function createLivePreviewMovieClip():void {
			...
			var Icon:*;
			try {
				Icon = flash.utils.getDefinitionByName("Icon");
